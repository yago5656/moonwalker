...
__global__ void comp_keys(uint8_t* gTableXCPU, uint8_t* gTableYCPU) {
...
__global__ void comp_keys_pattern(uint32_t mode, prefix_t *pattern, uint64_t *keys,  uint32_t maxFound, uint32_t *found, uint8_t* gTableXCPU, uint8_t* gTableYCPU) {

  int xPtr = (blockIdx.x*blockDim.x) * 8;
  int yPtr = xPtr + 4 * NB_TRHEAD_PER_GROUP;
//  ComputeKeys(mode, keys + xPtr, keys + yPtr, NULL, (uint32_t *)pattern, maxFound, found, gTableXCPU, gTableYCPU);
...
bool GPUEngine::callKernel(uint8_t* gTableXCPU, uint8_t* gTableYCPU) {

  // Reset nbFound
 // cudaMemset(outputPrefix,0,4);
    //printf("GPUEngine SetPrefix \n");
#define BLOCKS 32
#define THREADS_PER_BLOCK 256
    ...
    if (0 > 1) {
        ...
        if (searchType == P2SH) {
            ...
            if (hasPattern) {
                //printf("GPUEngine: (TODO) BECH32 not yet supported with wildard 3\n");
                ...
                //printf("GPUEngine: (TODO) BECH32 not yet supported with wildard 4\n");
            ...
            if (hasPattern) {
                ...
                comp_keys_pattern << < nbThread / NB_TRHEAD_PER_GROUP, NB_TRHEAD_PER_GROUP >> >
                    (searchMode, inputPrefix, inputKey, maxFound, outputPrefix, gTableXCPU, gTableYCPU);
            }
            else {
                //if (searchMode == SEARCH_COMPRESSED) {
                if (0) {
                    //  printf("GPUEngine: (TODO) BECH32 not yet supported with wildard 1\n");
                    ...
                    // printf("GPUEngine: (TODO) BECH32 not yet supported with wildard 2\n");
//                    comp_keys << < nbThread / NB_TRHEAD_PER_GROUP, NB_TRHEAD_PER_GROUP >> >
  //                      (searchMode, inputPrefix, inputPrefixLookUp, inputKey, maxFound, outputPrefix, gTableXCPU, gTableYCPU);
                }
            }
...
bool GPUEngine::SetKeys(Point *p, uint8_t* gTableXCPU, uint8_t* gTableYCPU) {

    callKernel(gTableXCPU, gTableYCPU);
  ...
  return "v";//callKernel(gTableXCPU, gTableYCPU);

}

bool GPUEngine::Launch(std::vector<ITEM>& prefixFound, uint8_t* gTableXCPU, uint8_t* gTableYCPU, bool spinWait) {
...
//if (1 != 1) {
  ...
  //callKernel();
  return callKernel(gTableXCPU, gTableYCPU);
